@extends('layouts.app')
@section('title', 'الإشعارات — استقبال')
...
<div class="container mx-auto max-w-3xl px-4 sm:px-6 py-8">
    <div class="flex items-center justify-between flex-wrap gap-3 mb-6">
        ...
        @if ($unreadBadge > 0)
            <form method="POST" action="{{ route('reception.notifications.read') }}">
                @csrf
            ...
            <div class="card p-4 {{ $n->is_read ? '' : 'border-primary/40 bg-primary/5' }}">
                <div class="flex items-start gap-3">
                    <div class="flex h-9 w-9 items-center justify-center rounded-xl shrink-0
                        {{ match($n->type) {
                            'NEW_REQUEST' => 'bg-blue-100 dark:bg-blue-950/40 text-blue-600 dark:text-blue-400',
                            'TECHNICIAN_ASSIGNED' => 'bg-purple-100 dark:bg-purple-950/40 text-purple-600 dark:text-purple-400',
                            'REQUEST_UPDATED' => 'bg-amber-100 dark:bg-amber-950/40 text-amber-600 dark:text-amber-400',
                            default => 'bg-muted text-muted-foreground',
                        } }}">
                        {!! icon(match($n->type) { 'NEW_REQUEST' => 'wrench', 'TECHNICIAN_ASSIGNED' => 'hard-hat', 'REQUEST_UPDATED' => 'refresh', default => 'bell' }, 'h-4 w-4') !!}
                    </div>
                    <div class="flex-1 min-w-0">
                        ...
                        @if ($n->request_id && $n->request)
                            <a href="{{ route('reception.show', $n->request_id) }}" class="text-xs text-primary hover:underline mt-1.5 inline-flex items-center gap-1">
