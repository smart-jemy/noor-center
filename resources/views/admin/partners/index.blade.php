...
@section('title', 'شركاء الصيانة — لوحة التحكم')
@section('admin_title', 'شركاء الصيانة')
@section('admin_subtitle', 'فنيون خارجيون — لكل شريك حساب مستقل: "له ما له وعليه ما عليه"')

@section('admin_actions')
    <button onclick="document.getElementById('partner-modal').showModal()" class="btn btn-primary btn-sm">{!! icon('plus', 'h-3.5 w-3.5') !!} شريك جديد</button>
@endsection
...
<div class="grid lg:grid-cols-2 gap-4">
    ...
    <div class="space-y-3">
        @forelse ($partners as $p)
            <div class="card p-5" x-data="{ tab: 'info', edit: false, repair: false, settle: false }">
                <div class="flex items-start justify-between gap-3 mb-3 flex-wrap">
                    ...
                    <div class="flex items-center gap-1.5">
                        ...
                        <form method="POST" action="{{ route('admin.partners.toggle', $p) }}">
                            @csrf
                            <button class="btn btn-outline btn-sm" title="{{ $p->is_active ? 'تعطيل' : 'تفعيل' }}">{!! icon('power', 'h-3.5 w-3.5') !!}</button>
                        </form>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-3 gap-2 mb-3 text-center">
                    <div class="p-2.5 rounded-xl bg-muted/40">
                        <div class="text-[10px] text-muted-foreground">إجمالي صياناته</div>
                        <div class="font-extrabold text-sm">{{ money($p->total_amount) }}</div>
                    </div>
                ...
                @if ($p->credit_limit > 0)
                    <div class="text-[10px] text-muted-foreground mb-3">
                        حد الآجل: {{ money($p->credit_limit) }} @if ($p->overLimit()) <span class="text-red-600 font-bold">— تجاوزه!</span> @endif
                    </div>
                @endif

                {{-- أزرار --}}
                <div class="flex flex-wrap gap-2">
                    <button @click="tab = 'info'; edit = false; repair = false; settle = false" class="btn btn-sm {{ $tab ?? '' }}" :class="tab === 'info' ? 'btn-primary' : 'btn-outline'">البيانات</button>
                    <button @click="tab = 'repair'; edit = false; settle = false" class="btn btn-sm" :class="tab === 'repair' ? 'btn-primary' : 'btn-outline'">{!! icon('wrench', 'h-3 w-3') !!} صيانة جديدة</button>
                    <button @click="tab = 'settle'; edit = false; repair = false" class="btn btn-sm" :class="tab === 'settle' ? 'btn-primary' : 'btn-outline'">{!! icon('banknote', 'h-3 w-3') !!} تسوية</button>
                    <button @click="tab = 'edit'; repair = false; settle = false" class="btn btn-sm" :class="tab === 'edit' ? 'btn-primary' : 'btn-outline'">{!! icon('edit', 'h-3 w-3') !!}</button>
                </div>
                ...
                <div class="mt-3">
                    ...
                    {{-- تعديل --}}
                    <form x-show="tab === 'edit'" x-transition method="POST" action="{{ route('admin.partners.update', $p) }}" class="space-y-2" style="display:none">
                        @csrf
                        @method('PUT')
                        <input name="name" value="{{ $p->name }}" class="input" required>
                        <input name="phone" value="{{ $p->phone }}" placeholder="الهاتف" class="input" dir="ltr">
                        <input name="credit_limit" type="number" step="0.01" min="0" value="{{ $p->credit_limit }}" placeholder="حد الآجل (0 = بدون)" class="input" dir="ltr">
                        <textarea name="notes" rows="2" placeholder="ملاحظات" class="input">{{ $p->notes }}</textarea>
                        <button class="btn btn-primary btn-sm w-full">حفظ</button>
                    </form>

                    {{-- صيانة جديدة — أجهزة متعددة + سعر اختياري --}}
                    <form x-show="tab === 'repair'" x-transition method="POST" action="{{ route('admin.partners.repairs', $p) }}" class="space-y-3" style="display:none"
                          x-data="{ devices: [{ device_type: '', brand: '', customer_name: '', issue_description: '', total_amount: '', paid_amount: '' }] }">
                        @csrf
                    ...
                    {{-- تسوية --}}
                    <form x-show="tab === 'settle'" x-transition method="POST" action="{{ route('admin.partners.settlements', $p) }}" class="space-y-2" style="display:none">
                        @csrf
                        <div class="grid grid-cols-2 gap-2">
            ...
            <div class="space-y-2">
                @forelse ($recentRepairs as $r)
                    <div class="p-3 rounded-xl bg-muted/40 text-sm">
                        ...
                        @if (! $r->hasPrice())
                            <form method="POST" action="{{ route('admin.partners.repairs.update', $r) }}" class="mt-2 flex items-center gap-1.5">
                                @csrf @method('PUT')
...
<dialog id="partner-modal" class="modal-backdrop">
    <form method="POST" action="{{ route('admin.partners.store') }}" class="modal-box max-w-md">
        @csrf
